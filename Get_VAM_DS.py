...
class re_start_class():
    ...
    def load(self):
        with open('re_start.json', 'r') as fp:
            self.data = json.load(fp)
            
    def save(self):
        with open('re_start.json', 'w') as fp:
            json.dump(self.data, fp ,indent=4)

    def update_data(self,Connection,OD,wt,GradeOrigin,Grade):

        if Connection != re_start.data['Last_Connection']:
            re_start.data['Loaded_Connection'].append(re_start.data['Last_Connection'])
            re_start.data['Last_Connection']=Connection

        if OD != re_start.data['Last_OD']:
            re_start.data['Loaded_OD'].append(re_start.data['Last_OD'])
            re_start.data['Last_OD'] = OD

        if wt != re_start.data['Last_wt']:
            re_start.data['Loaded_wt'].append(re_start.data['Last_wt'])
            re_start.data['Last_wt'] = wt

        if GradeOrigin != re_start.data['Last_GradeOrigin']:
            re_start.data['Loaded_GradeOrigin'].append(re_start.data['Last_GradeOrigin'])
            re_start.data['Last_GradeOrigin'] = GradeOrigin
            re_start.data['Loaded_Grade']=[]

        if Grade != re_start.data['Last_Grade']:
            re_start.data['Loaded_Grade'].append(re_start.data['Last_Grade'])
            re_start.data['Last_Grade'] = Grade

def DataFrameToRow(df):
...
def GetInfoFromNavegador(Navegador):
    Navegador.wait_load()
    ...
    ConnectionReferenceVME=Navegador.driver.find_element(By.ID,"ctl00_ContentPlaceHolder1_Uc_datasheet_view_new1_DataList_CDS_ctl00_Img_Marketing")
    FootNotes=Navegador.driver.find_element(By.ID,"ctl00_ContentPlaceHolder1_Uc_datasheet_view_new1_DataList_CDS_ctl00_Lbl_Marketing")
    ...
    for var in [PipeProperties,ConnectionProperties,ConnectionPerformance,ConnectionTorques]:
        ...
        row_data=row_data.append(DataFrameToRow(pd.DataFrame(read_html)).T,ignore_index=False)
    row_data = row_data.T
                    ...
                    for Grade in list_Grade:
                        ...
                        for Design in list_Design:
                            ...
                            for CouplingOption in list_CouplingOption:
                                ...
                                row_data=GetInfoFromNavegador(Navegador)
